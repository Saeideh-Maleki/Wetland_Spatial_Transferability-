...
def testModel(model, test, loss_fn, device, dates=None):
    ...
    labels, pred_all, probs_all = [], [], []  # for f1-score
    with torch.no_grad():
        for x_batch, y_batch in test:
            ...
            # Softmax probs
            probs_all.append(F.softmax(pred, dim=1).cpu().numpy())
...
def trainTestModel(model_name, file_path, x_train, x_test, y_train, y_test, dates_train, dates_test=None, n_epochs=50, batch_size=16):
    ...
    if model_name == 'RF':
        ...
        y_pred, y_probs = testModel(model, test_dataloader, loss, device, dates_test)
...
def printMeasures(y_pred,y_real,verbose=True):
    ...
    print(f"Accuracy={(100*accuracy):.3f}%,Kappa={kappa:.4f}",
          
          f"F1={f1.mean()*100:.3f} (class1: {f1[0]*100:.3f}, class2: {f1[1]*100:.3f}, class3: {f1[2]*100:.3f}")
    # Precision - recall -  kappa - confusion matrix
    if verbose:
        precision, recall, f1, _ = precision_recall_fscore_support(y_real,y_pred)
        print(f"class1: Precision={100*precision[0]:.3f}%, Recall={100*recall[0]:.3f}%")
        print(f"class2: Precision={100*precision[1]:.3f}%, Recall={100*recall[1]:.3f}%")
        print(f"class3: Precision={100*precision[2]:.3f}%, Recall={100*recall[2]:.3f}%")
        ...
        print("\nConfusion matrix:")
        print(f"[class1] [{cm[0,0]:5d}, {cm[0,1]:5d}, {cm[0,2]:5d}]")
        print(f"[class2] [{cm[1,0]:5d}, {cm[1,1]:5d}, {cm[1,2]:5d}]")
        print(f"[class3] [{cm[2,0]:5d}, {cm[2,1]:5d}, {cm[2,2]:5d}]")


 
        class_names = ['AV', 'OW','TV']

# Create a heatmap with red color map
        plt.figure(figsize=(10, 7))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Reds', xticklabels=class_names, yticklabels=class_names)
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.title('Confusion Matrix with Red Color Map')
        plt.show()
